...
INTEGER, SAVE, VOLATILE :: IERROR = 0
CHARACTER (LEN=16) :: CLARG
INTEGER :: NSTOP = 10000
INTEGER*8, SAVE :: NSEED = 2713
...
IF (COMMAND_ARGUMENT_COUNT () > 1) THEN
  CALL GET_COMMAND_ARGUMENT (2, CLARG)
  READ (CLARG, *) NSEED
ENDIF
...
NSEED = MODULO (16807_8 * NSEED, 2147483647_8)

IRAND = K1 + MODULO (NSEED, INT (K2-K1+1, 8))
...
IF (.NOT. ASSOCIATED (YLF4)) THEN
  ...
  SELECT CASE (IRAND (1, 2))
    CASE (1)
      CALL NEW_OWNER_YLF4 (IRAND (0, 1))
    CASE (2)
      CALL NEW_WRAPPER_YLF4 (IRAND (0, 1))
    CASE DEFAULT
...
SUBROUTINE NEW_OWNER_YLF4 (KPART)

INTEGER (KIND=JPIM), INTENT (IN) :: KPART

CALL PLOG ("NEW_OWNER_YLF4", [KPART])

CALL FIELD_NEW (YLF4, CHILDREN=YLF3L, UBOUNDS=IUB, LBOUNDS=ILB, PERSISTENT=LLPERSISTENT, PARTIAL=KPART>0)

END SUBROUTINE

SUBROUTINE NEW_WRAPPER_YLF4 (KPART)

INTEGER (KIND=JPIM), INTENT (IN) :: KPART

CALL PLOG ("NEW_WRAPPER_YLF4", [KPART])

CALL FIELD_NEW (YLF4, CHILDREN=YLF3L, LBOUNDS=ILB, PERSISTENT=LLPERSISTENT, DATA=ZDATA4, PARTIAL=KPART>0)
...
IF (LLVERBOSE) THEN
  ...
  WRITE (*, '(I6.6," | ",I12.12," | ")', ADVANCE='NO') NSTEP, NSEED
